...
test("query", function(t) {
  ...
  let table = [{ id: 1 }, { id: 2 }, { id: "abc" }];

  let table2 = [{ foo: "a" }, { foo: "b" }, { foo: "b" }];
  ...
  t.deepEqual(query(table, { id: { $gte: 1, $lte: 2 } }), [
    { id: 1 },
    { id: 2 }
  ]);
  ...
  t.deepEqual(
    query(table2, { foo: { $in: ["a"] } }),
    table2.filter(d => d.foo == "a")
  );
  t.deepEqual(
    query(table2, { foo: { $in: ["b"] } }),
    table2.filter(d => d.foo == "b")
  );
  t.deepEqual(query(table2, { foo: { $in: ["a", "b"] } }), table2);

  t.deepEqual(query(table, { $and: [{ id: 1 }, { id: { $ne: "abc" } }] }), [
    { id: 1 }
  ]);
  t.deepEqual(query(table, { $or: [{ id: 1 }, { id: 2 }] }), [
    { id: 1 },
    { id: 2 }
  ]);
  ...
  t.deepEqual(
    query([{ id: 1 }, { foo: 1 }], {
      $or: [{ id: { $exists: true } }, { foo: 1 }]
    }),
    [{ id: 1 }, { foo: 1 }]
  );
  t.deepEqual(
    query([{ id: 1, foo: 1 }, { foo: 1 }], {
      $and: [{ id: { $exists: true } }, { foo: 1 }]
    }),
    [{ id: 1, foo: 1 }]
  );
